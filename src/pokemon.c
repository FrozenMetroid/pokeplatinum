...
static void BoxPokemon_SetDefaultMoves(BoxPokemon *boxMon)
{
    ...
    if (monSpecies == SPECIES_SMEARGLE) { // Smeargle always starts with Sketch and then has 3 empty move slots
        BoxPokemon_SetMoveSlot(boxMon, MOVE_SKETCH, 0);
        for (i = 1; i < LEARNED_MOVES_MAX; i++) {
            BoxPokemon_SetMoveSlot(boxMon, MOVE_NONE, i);
        }
    } else {
