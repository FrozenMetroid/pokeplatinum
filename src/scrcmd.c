...
static BOOL FieldTask_WonderTrade(FieldTask *task);
...
static BOOL ScrCmd_WonderTrade(ScriptContext *ctx)
{
    struct WonderTradeData *taskEnv = Heap_Alloc(HEAP_ID_FIELD2, sizeof(struct WonderTradeData));

    memset(taskEnv, 0, sizeof(struct WonderTradeData));
    taskEnv->level =                ScriptContext_GetVar(ctx);
    taskEnv->partySlot =            ScriptContext_GetVar(ctx);
    taskEnv->natDexWonderTrade =    ScriptContext_GetVar(ctx);
    taskEnv->finalMessage =         ScriptContext_GetVarPointer(ctx);
    taskEnv->party =                SaveData_GetParty(ctx->fieldSystem->saveData);
    Pokemon_Copy(Party_GetPokemonBySlotIndex(taskEnv->party, taskEnv->partySlot), &taskEnv->sentPokemon);
    Party_RemovePokemonBySlotIndex(taskEnv->party, taskEnv->partySlot);
    taskEnv->partySlot = Party_GetCurrentCount(taskEnv->party);
...
// don't want to deal with overlays for now so just put it here
