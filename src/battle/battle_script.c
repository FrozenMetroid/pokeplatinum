...
static BOOL BtlCmd_CheckFlingExceptions(BattleSystem *battleSys, BattleContext *battleCtx);
...
static void BattleScript_CalcMoveDamage(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    BattleContext_ReduceCriticalDamage(battleCtx);
    #endif
...
static BOOL BtlCmd_CalcExpGain(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    if ((battlerData->battlerType & BATTLER_TYPE_SOLO_ENEMY) && (battleType & BATTLE_TYPE_NO_EXPERIENCE) == FALSE) {
        ...
        battleCtx->slotNeedsExpMessage = 0; // reset this
        ...
        for (i = 0; i < partyCount; i++) {
            ...
            if (Pokemon_GetValue(mon, MON_DATA_SPECIES, NULL) && Pokemon_GetValue(mon, MON_DATA_HP, NULL)) {
                if (battleCtx->sideGetExpMask[battler] & FlagIndex(i)) {
                    totalMonsGainingExp++;
                }
                if (partyWideExpShare) { // need special handling for this

                    // now figure out which mons need their exp message to be displayed individually
                    ...
                    if (BattleSystem_PokemonIsOT(battleSys, mon) == FALSE
                        ...
                        battleCtx->slotNeedsExpMessage |= (1 << i);
                    }
                }
            }
        }
        ...
        battleCtx->gainedExp = exp / totalMonsGainingExp; // if the mons actively participated, you still need to divide by the number of them

        if (battleCtx->gainedExp == 0) {
            battleCtx->gainedExp = 1;
        }
        if (partyWideExpShare) {
            battleCtx->sharedExp = battleCtx->gainedExp / 3; // reduce party wide exp from half to a third because Sinnoh was not designed with this feature in mind and I don't want to go through and change all of the levels
...
static BOOL BtlCmd_ChangeStatStage(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    if (stageChange > 0) {
        ...
        if (battleCtx->sideEffectType == SIDE_EFFECT_TYPE_ABILITY) {
            ...
            // TODO: Confirm this change works for everything
            if (battleCtx->attacker != battleCtx->msgBattlerTemp) { // added for Weak Armor
                battleCtx->msgBuffer.params[0] = BattleSystem_NicknameTag(battleCtx, battleCtx->msgBattlerTemp);
                battleCtx->msgBuffer.params[1] = battleCtx->battleMons[battleCtx->msgBattlerTemp].ability;
            } else {
...
static BOOL BtlCmd_Transform(BattleSystem *battleSys, BattleContext *battleCtx)
{
    ...
    ATTACKING_MON.dampAnnounced = FALSE;
...
static void BattleScript_GetExpTask(SysTask *task, void *inData)
{
    ...
    BOOL showPartyWideExpGainMessage = FALSE;
    u32 curExp;
    ...
    if (!partyWideExpShare) {
        for (slot = data->tmpData[GET_EXP_PARTY_SLOT]; slot < partyCount; slot++) {
            ...
            if ((data->battleCtx->sideGetExpMask[battler] & FlagIndex(slot))) { // if party-wide exp share, give exp to every slot; otherwise, only slots that participated
                break;
            }
        }
    } else {
        ...
        if (data->battleCtx->slotNeedsExpMessage & (1 << data->battleCtx->selectedPartySlot[expBattler])) {
            ...
            #endif
            for (i = 0; i < partyCount; i++) {
                if (data->battleCtx->slotNeedsExpMessage & (1 << i)) {
                    slot = i;
                    #ifdef DEBUG_EXP_GAIN
                    EmulatorLog("Slot gets boosted exp: %d", slot);
                    #endif
                    break;
                }
            }
        } else if (data->battleCtx->remainingExpShareSlots) { // find the next slot that didn't get a special exp message from above
            // this ensures that the "party gained exp" message shows up AFTER the individualized messages and BEFORE we see the remaining slots level up, if applicable, and only shows up once
            if (!data->battleCtx->shownPartyGainedExpMsg) {
                showPartyWideExpGainMessage = TRUE;
            }
            for (i = 0; i < partyCount; i++) {
                if (data->battleCtx->remainingExpShareSlots & (1 << i)) {
                    slot = i;
                    break;
                }
            }
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Non-special slot chosen with exp share");
            #endif
        } else {
            slot = partyCount; // if no slots need messages/exp, set slot to party count to end the sequence
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("No more slots need EXP messages");
            #endif
        }
        // redundant for the "else" case above but needed for the other two
        mon = BattleSystem_GetPartyPokemon(data->battleSys, expBattler, slot);
    }

    if (slot == partyCount) { // if we've gone through every slot, end the sequence
        #ifdef DEBUG_EXP_GAIN
        EmulatorLog("Ending EXP Gain, slot = %u", slot);
        #endif
    ...
    case SEQ_GET_EXP_START:
        ...
        if (Pokemon_GetValue(mon, MON_DATA_HP, NULL) && Pokemon_GetValue(mon, MON_DATA_LEVEL, NULL) != MAX_POKEMON_LEVEL && Pokemon_GetValue(mon, MON_DATA_IS_EGG, NULL) == FALSE) {
            if (partyWideExpShare == FALSE && data->battleCtx->sideGetExpMask[battler] & FlagIndex(slot)) {
                ...
                if (slot == data->battleCtx->selectedPartySlot[expBattler]
                    || (data->battleCtx->sideGetExpMask[battler] & FlagIndex(slot))) {
                        totalExp = data->battleCtx->gainedExp;
                        #ifdef DEBUG_EXP_GAIN
                        EmulatorLog("Calculating exp gain for slot %d since it participated in the battle", slot);
                        #endif
                    }
                else {
                    totalExp = data->battleCtx->sharedExp;
                    #ifdef DEBUG_EXP_GAIN
                    EmulatorLog("Calculating exp gain for slot %d due to party-wide exp share", slot);
                    #endif
                }
            }
            ...
            if (battleType & BATTLE_TYPE_TRAINER) {
                totalExp = totalExp * 150 / 100;
                #ifdef DEBUG_EXP_GAIN
                EmulatorLog("Applying trainer battle exp boost of 1.5x");
                #endif
            }
            ...
            curExp = Pokemon_GetValue(mon, MON_DATA_EXPERIENCE, NULL);
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Total Current EXP: %d", curExp);
            #endif
            u32 baseEXPForLevel = Pokemon_GetCurrentLevelBaseExp(mon);
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Base EXP for current level: %d", baseEXPForLevel);
            #endif
            data->tmpData[GET_EXP_NEW_EXP] = curExp - baseEXPForLevel; // this gives the amount of exp that the mon is currently displaying on the bar
            u32 newEXP = curExp + totalExp;
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("EXP given: %d", newEXP - curExp);
            #endif

            // set the active slot's exp in the battle context to the new exp
            if (slot == data->battleCtx->selectedPartySlot[expBattler]) {
        ...
        if (totalExp) {
            ...
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Slot doesn't get exp");
            #endif
    ...
    case SEQ_GET_EXP_WAIT_MESSAGE_PRINT:
        if (Text_IsPrinterActive(data->tmpData[GET_EXP_MSG_INDEX]) == FALSE) {
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Text printer inactive, going to state SEQ_GET_EXP_WAIT_MESSAGE_DELAY");
            #endif
            data->seqNum++;
        }
        break;
    ...
    case SEQ_GET_EXP_WAIT_MESSAGE_DELAY:
        if (--data->tmpData[GET_EXP_MSG_DELAY] == 0) {
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Message delay finished, going to state SEQ_GET_EXP_GAUGE");
            #endif
            data->seqNum++;
        }
        break;
    ...
    case SEQ_GET_EXP_GAUGE:
        ...
        if (slot == data->battleCtx->selectedPartySlot[expBattler]) {
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Slot is the active battler, show the EXP bar animation");
            #endif
            ...
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Setting seqNum to SEQ_GET_EXP_CHECK_LEVEL_UP");
            #endif
            data->seqNum = SEQ_GET_EXP_CHECK_LEVEL_UP;
        }
        break;
    ...
    case SEQ_GET_EXP_CHECK_DONE:
        ...
        data->battleCtx->slotNeedsExpMessage &= ~(1 << slot); // clear the flag for the slot
        data->battleCtx->remainingExpShareSlots &= ~(1 << slot); // clear the flag for the slot
        #ifdef DEBUG_EXP_GAIN
        EmulatorLog("Moving on to the next slot");
        #endif
        break;
    ...
    case SEQ_GET_EXP_DONE:
        if (Text_IsPrinterActive(data->tmpData[GET_EXP_MSG_INDEX]) == FALSE) {
            ...
            SysTask_Done(task);
            #ifdef DEBUG_EXP_GAIN
            EmulatorLog("Done with Get EXP sequence");
            #endif
        }
        break;
    }
...
static int BattleScript_CalcCatchShakes(BattleSystem *battleSys, BattleContext *battleCtx)
{
    if (BattleSystem_GetBattleType(battleSys) & BATTLE_TYPE_ALWAYS_CATCH
        || battleCtx->msgItemTemp == ITEM_MASTER_BALL) {
