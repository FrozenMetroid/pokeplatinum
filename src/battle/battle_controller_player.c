        ...
        case PRE_MOVE_ACTION_STATE_CHECK_RAGE_FLAG:
            for (battler = 0; battler < maxBattlers; battler++) {
                if (battleCtx->battleMons[battler].statusVolatile & VOLATILE_CONDITION_RAGE
                    ...
                    // FIX: no longer clears every volatile status except rage
...
{
    if (MOVE_DATA(move).type == TYPE_NORMAL && MoveIsAffectedByNormalizeVariants(move)) {
...
{
    if (CURRENT_MOVE_DATA.type == TYPE_NORMAL && MoveIsAffectedByNormalizeVariants(battleCtx->moveCur)) {
