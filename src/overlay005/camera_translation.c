...
static void stepangle(Camera *camera, const u16 *first, const u16 *last, u8 step, u8 duration);
static void stepDistance(Camera *camera, const fx32 *first, const fx32 *last, u8 step, u8 duration);
static void stepfovY(Camera *camera, const u16 *first, const u16 *last, u8 step, u8 duration);
...
void SetCameraTranslationPath(GFCameraTranslationWrapper *wrapper, struct CameraTranslationPathTemplate *template, int duration) {
    ...
    if (!wrapper->active && duration != 0) {
        ...
        angle = Camera_GetAngle(wrapper->camera);
        target = Camera_GetTarget(wrapper->camera);
...
static void stepCamera(Camera *camera, struct CameraTranslationPathTemplate *first, struct CameraTranslationPathTemplate *last, u8 step, u8 duration) {
    ...
    stepangle(camera, &first->angle, &last->angle, step, duration);
    stepfovY(camera, &first->fovY, &last->fovY, step, duration);
...
static void stepangle(Camera *camera, const u16 *first, const u16 *last, u8 step, u8 duration) {
    ...
    Camera_SetAngleAroundTarget(&cameraAngle, camera);
}
