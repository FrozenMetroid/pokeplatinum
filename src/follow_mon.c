...
static BOOL FollowMon_GetPermissionBySpeciesAndMap(u16 species, u16 mapNo);
...
static const u16 sFollowMonHasGenderDiff[] = {
...
BOOL FollowMon_HasGenderDifference(u16 species)
{
    ...
    return sFollowMonHasGenderDiff[species];
}

u16 FollowMon_SpeciesToGFXID(u16 species)
{
...
MapObject *FollowMon_InitMapObject(MapObjectManager *mapObjectManager, int x, int y, int direction, u32 mapNo)
{
    ...
    FollowMon_SaveSetUnused2bitField(0, FollowMon_SaveGet(fieldSystem->saveData));
...
BOOL FollowMon_GetPermissionBySpeciesAndMap(u16 species, u16 mapNo)
{
    BOOL ret;
...
static BOOL FollowMon_DiglettPermissionCheck(u16 mapNo)
{
    // add specific maps diglett and dugtrio shouldn't appear on
...
u8 FollowMon_GetSizeParamBySpecies(u16 species)
{
    u8 buf[4]

    NARC_ReadWholeMemberByIndexPair(buf, NARC_INDEX_POKETOOL_FOLLOWMONDATA, FollowMon_SpeciesToGFXID(species));
